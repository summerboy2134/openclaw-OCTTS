...
class TrackedRecommendationState(BaseModel):
    ...
    frontlist_rank: Optional[int] = Field(None, description="展示池排序位次")
    ...
    final_selection_score: Optional[float] = Field(None, description="最终排序分")
    ...
    close_auction_price: Optional[float] = Field(None, description="收盘集合竞价成交价")
    close_auction_amount: Optional[float] = Field(None, description="收盘集合竞价成交额")
    close_auction_vwap: Optional[float] = Field(None, description="收盘集合竞价均价")
    close_auction_price_deviation_pct: Optional[float] = Field(None, description="收盘竞价价差百分比")
    close_auction_amount_ratio: Optional[float] = Field(None, description="收盘竞价成交额占全天成交额比例")
    stage3_close_auction_score: Optional[float] = Field(None, description="Stage3收盘竞价修正分")
    stage3_close_auction_flags: List[str] = Field(default_factory=list, description="收盘竞价正向标签")
    stage3_close_auction_risks: List[str] = Field(default_factory=list, description="收盘竞价风险标签")
    stage3_close_auction_veto: bool = Field(False, description="是否触发收盘竞价假强过滤")
    stage3_close_auction_missing: bool = Field(False, description="收盘竞价数据是否缺失")
